...
const projectId = useRuntimeConfig().public.wCProjectId
...
const networkName = computed(() => {
  if (chain.value) {
    return (
      chain.value.name || chainIdToName[chain.value.id] || 'Unknown Network'
    )
  }
  return 'Loading...'
})
...
const handleWalletLogin = async (address: string) => {
  ...
  try {
    ...
    await signMessage({ message: nonce })
    ...
    watchEffect((onInvalidate) => {
      ...
      const verifySignature = async () => {
        ...
        const verifyResponse = await verifyMessage(nonce, signature, address)
