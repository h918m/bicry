...
const currencyDecimals = computed(() => {
  return tokens.value.find((token: any) => token.currency === props.currency)
    ?.decimals
})

const pairDecimals = computed(() => {
  return tokens.value.find((token: any) => token.currency === props.pair)
    ?.decimals
})
