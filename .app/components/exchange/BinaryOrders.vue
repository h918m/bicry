...
const isPractice = route.query.isPractice === 'true' ? true : false || false

onMounted(async () => {
  if (userStore.isLoggedIn) {
    if (marketStore.binaryPositions.length === 0) {
...
const orders = computed(() => {
  return marketStore.binaryPositions.filter(
    (item) => item.is_demo === isPractice,
  )
})
