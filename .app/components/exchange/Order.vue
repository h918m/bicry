...
onMounted(async () => {
  if (userStore.isLoggedIn) {
    ...
    if (
      walletStore.wallets[props.currency] === undefined ||
      walletStore.wallets[props.currency] === null ||
      (walletStore.wallets[props.currency] &&
        walletStore.wallets[props.currency].type !== 'SPOT')
    ) {
      await createWallet(props.currency)
    }
    if (
      walletStore.wallets[props.pair] === undefined ||
      walletStore.wallets[props.pair] === null ||
      (walletStore.wallets[props.pair] &&
        walletStore.wallets[props.pair].type !== 'SPOT')
    ) {
...
const currencyWallet = computed(() => {
  return walletStore.wallets[props.currency] || null
})
...
watch(
  ...
  (newVal) => {
    ...
    const availableBalance =
      walletStore.wallets[isBuySide ? props.pair : props.currency]?.balance || 0
        ...
        >
          ...
          <div class="px-2">
            ...
            >
              <span
                v-if="
                  !walletStore.wallets[currency] || !walletStore.wallets[pair]
                "
