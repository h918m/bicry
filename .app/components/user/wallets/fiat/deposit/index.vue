...
const taxedAmount = computed(() =>
  percentageTaxesRate.value > 0 || fixedTaxesRate.value > 0
    ? (selectedMethod.value?.alias ? paymentAmount.value : values.amount) *
        (percentageTaxesRate.value > 0 ? percentageTaxesRate.value : 1) +
      fixedTaxesRate.value
    : 0,
)
    ...
    try {
      const response = await $fetch('/api/ipn/stripe', {
        ...
        body: JSON.stringify({
          ...
          taxAmount: taxedAmount.value * 100,
      ...
      >
        ...
        >
          ...
          >
            {{
              Number(selectedMethod?.percentage_fee) > 0
                ? `Taxes are calculated at a flat rate of ${Number(
