...
const { getWallet, getDepositAddress } = useEcosystem()
...
watch(selectedChain, async (newChain, oldChain) => {
  ...
  if (oldChain) {
    ...
    if (token.value?.contractType === 'NO_PERMIT') {
      oldDepositAddr = dynamicDepositAddress.value // We use the dynamically fetched address
    } else {
  ...
  if (newChain) {
    ...
    if (newDepositAddr) {
      if (!isDepositSocketOpen.value) {
        await ecoSocketStore.createSocket('deposits')
...
function unsubscribeAndCloseSocket() {
  ...
  ecoSocketStore.unsubscribe('deposits', subscription)
  ...
  <div>
    <div v-if="deposit && !deposit.status">
      <div class="my-auto text-center flex-col justify-center space-y-5">
        ...
        <p>
          {{ $t('Transaction received. Please wait while we process it') }}.
        </p>
      </div>
    </div>
