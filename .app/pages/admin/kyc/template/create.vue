...
definePageMeta({
  title: 'Create KYC Template',
  permissions: ['Create KYC Template'],
})

const kycTemplateStore = useKycTemplateStore()
const { createKycTemplate } = useKyc()
...
const customFields = ref<FormInput['options']['custom_fields']>([])
...
const update = handleSubmit(async (values: FormInput) => {
  ...
  try {
    const response = await createKycTemplate(values)
...
<template>
  <TairoContentWrapper>
    <template #left>
      <BaseHeading size="lg">{{ $t('Create Template') }}</BaseHeading>
    </template>
    <template #right>
