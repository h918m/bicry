...
definePageMeta({
  ...
  permissions: ['Edit Withdraw Methods'],
})
// Assuming you have the ID of the method you want to edit
const id = useRoute().params.id
const withdrawMethodStore = useWithdrawMethodStore()
const { updateWithdrawMethod } = useWallet()
const existingMethod = computed(
  () =>
    withdrawMethodStore.selectedMethod || withdrawMethodStore.getMethodById(id),
)
...
onMounted(async () => {
  ...
  withdrawMethodStore.selectMethodById(methodId)
  imagePreviewUrl.value = existingMethod.value?.image || null
  if (!withdrawMethodStore.selectedMethod) {
...
const addCustomField = () => {
  ...
  setFieldValue('custom_fields', updatedCustomFields) // Update the form values
}
...
const update = handleSubmit(async (values: FormInput) => {
  try {
    ...
    if (imageFile.value) {
      ...
      body.append('type', 'withdrawMethods')
    ...
    const response = await updateWithdrawMethod(id, data)
...
<template>
  <TairoContentWrapper>
    <template #left>
      <BaseHeading size="lg">{{ $t('Edit Withdraw Method') }}</BaseHeading>
    </template>
    <template #right>
      <BaseButton
        type="button"
