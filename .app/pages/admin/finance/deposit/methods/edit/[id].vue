...
definePageMeta({
  ...
  permissions: ['Edit Deposit Methods'],
})
// Assuming you have the ID of the method you want to edit
const id = useRoute().params.id
const depositMethodStore = useDepositMethodStore()
const { updateDepositMethod } = useWallet()
const existingMethod = computed(
  () =>
    depositMethodStore.selectedMethod || depositMethodStore.getMethodById(id),
)

// Image File Handling
const imageFile = ref<FileList | null>(null)
const imagePreviewUrl = ref<string | null>(existingMethod.value?.image || null)
...
onMounted(async () => {
  ...
  depositMethodStore.selectMethodById(methodId)
  imagePreviewUrl.value = existingMethod.value?.image || null
  if (!depositMethodStore.selectedMethod) {
    router.push('/admin/finance/deposit/methods')
  }
})
...
const initialValues = computed<FormInput>(() => ({
  ...
  custom_fields: existingMethod.value?.custom_fields || [],
}))
...
const customFields = computed<CustomField[]>(() => {
  return existingMethod.value?.custom_fields || []
})
...
const addCustomField = () => {
  ...
  const updatedCustomFields: CustomField[] = [...customFields.value, newField]

  // Update Vuex Store here
  depositMethodStore.updateCustomFields(updatedCustomFields)
}

// Method to remove a custom field
const removeCustomField = (index: number) => {
  const updatedCustomFields = customFields.value.slice()
...
const update = handleSubmit(async (values: FormInput) => {
  try {
    ...
    if (imageFile.value) {
      ...
      body.append('type', 'depositMethods')
    ...
    const response = await updateDepositMethod(id, data)
...
<template>
  <TairoContentWrapper>
    <template #left>
      <BaseHeading size="lg">{{ $t('Edit Deposit Method') }}</BaseHeading>
    </template>
    <template #right>
      <BaseButton
        type="button"
        color="muted"
        to="/admin/finance/deposit/methods"
      >
