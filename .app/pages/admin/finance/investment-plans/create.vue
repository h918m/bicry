...
definePageMeta({
  ...
  permissions: ['Create Investment Plans'],
})
const { toast } = useUtils()
const router = useRouter()

const investmentPlanStore = useInvestmentPlanStore()
const { createInvestmentPlan } = useInvestment()
...
const create = handleSubmit(async (values: FormInput) => {
  try {
    ...
    if (imageFile.value) {
      ...
      body.append('type', 'investmentPlan')
    ...
    const response = await createInvestmentPlan(data)
...
<template>
  <TairoContentWrapper>
    <template #left>
      <BaseHeading size="lg">{{ $t('Create Investment Plan') }}</BaseHeading>
    </template>
    <template #right>
      <BaseButton
        type="button"
