...
const items = computed(
  () =>
    faqEntriestore.faqs?.filter(
      (item) => item.question?.includes(filter.value),
    ),
)
const paginatedItems = computed(() => {
  const start = (page.value - 1) * perPage.value
  const end = start + perPage.value
  return items.value.slice(start, end).map((faq) => ({
    ...faq,
    question:
...
function selectEntry(item: FaqEntry) {
  faqEntriestore.selectFaq(item)
  router.push(`/admin/extensions/faq/entries/edit/${item.id}`)
}
...
async function deleteEntry() {
  ...
  try {
    const response = (await deleteAdminFaq(selectedItem.value?.id)) as any
  ...
  <div>
    <TairoContentWrapper>
      ...
      <template #right>
        <BaseButton
          color="primary"
          :to="`/admin/extensions/faq/entries/create`"
        >
      ...
      <div>
        <template
          v-if="!faqEntriestore.loading && paginatedItems?.length === 0"
        >
          <BasePlaceholderPage
            :title="
            ...
            >
              ...
              <template #end>
                <TableFlexTableCell
                  label="Category"
                  :hide-label="index > 0"
                  class="w-28 xs:w-full text-sm"
                >
                  {{ item.category?.identifier ?? 'N/A' }}
                </TableFlexTableCell>
