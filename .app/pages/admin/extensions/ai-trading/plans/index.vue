...
definePageMeta({
  permissions: ['View AI Trading Investment Plans'],
  title: 'AI Trading Plans',
})

const aiTradingPlanStore = useAiTradingPlanStore()
const { deleteAITradingPlan } = useAiTrading()
...
const isAllVisibleSelected = computed(
  () => selected.value.length === aiTradingPlanStore.plans.length,
)

const items = computed(() =>
  aiTradingPlanStore.plans.filter((item) => item.title.includes(filter.value)),
)
...
onMounted(async () => {
  if (aiTradingPlanStore.plans.length === 0) {
    aiTradingPlanStore.loading = true
    await aiTradingPlanStore.fetchAiTradingPlans()
    aiTradingPlanStore.loading = false
  }
})
const { toast } = useUtils()

function updateSelectedAiTradingPlanStatus(status: boolean) {
  aiTradingPlanStore
    .updateAiTradingPlansStatus(selected.value, status)
    .then((response) => {
...
function selectPlan(item: AiTradingPlan) {
  aiTradingPlanStore.selectPlan(item)
  router.push(`/admin/extensions/ai-trading/plans/edit/${item.id}`)
}
...
async function deletePlan() {
  ...
  try {
    const response = await deleteAITradingPlan(selectedMethod.value?.id)
    toast.response(response)
    if (response.status === 'success') {
      aiTradingPlanStore.removePlan(selectedMethod.value?.id)
    }
  } catch (error) {
  ...
  <div>
    <TairoContentWrapper>
      ...
      <template #right>
        <BaseButton
          color="primary"
          :to="`/admin/extensions/ai-trading/plans/create`"
        >
      ...
      <div>
        <template
          v-if="!aiTradingPlanStore.loading && paginatedItems?.length === 0"
        >
          <BasePlaceholderPage
            :title="
        ...
        <div v-else class="w-full">
          <div
            ...
            v-if="!aiTradingPlanStore.loading && selected.length > 0"
          >
            <span class="text-sm">
            ...
            <div class="flex gap-2">
              <TairoButtonIcon
                ...
                @click="() => updateSelectedAiTradingPlanStatus(false)"
              >
              ...
              <TairoButtonIcon
                ...
                @click="() => updateSelectedAiTradingPlanStatus(true)"
              >
            ...
            >
              ...
              <template #end>
                <TableFlexTableCell
                  ...
                  >{{ item.ai_trading_plan_duration?.length ?? 0 }}
                </TableFlexTableCell>
                <TableFlexTableCell
                  label="Status"
                  :hide-label="index > 0"
