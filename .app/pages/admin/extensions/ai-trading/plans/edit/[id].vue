...
import { useAiTradingDurationStore } from '~~/store/extensions/ai-trading/admin/durations'
import { toTypedSchema } from '@vee-validate/zod'

definePageMeta({
  title: 'Edit AI Trading Investment Plans',
  permissions: ['Update AI Trading Plans'],
})
...
const aiTradingDurationStore = useAiTradingDurationStore()
const { updateAITradingPlan } = useAiTrading()

const existingPlan = computed(
  () => aiTradingPlanStore.selectedPlan || aiTradingPlanStore.getPlanById(id),
)
...
const durations = computed(() =>
  aiTradingDurationStore.durations.map((item) => ({
...
onMounted(async () => {
  ...
  await aiTradingPlanStore.selectPlanById(methodId)
  imagePreviewUrl.value = existingPlan.value?.image || null
  if (!aiTradingPlanStore.selectedPlan) {
    router.push('/admin/extensions/ai-trading/plans')
  }
  if (aiTradingDurationStore.durations.length === 0) {
    await aiTradingDurationStore.fetchAiTradingDurations()
  }
  if (existingPlan.value?.ai_trading_plan_duration) {
    selectedDurations.value = durations.value.filter(
      (duration) =>
        existingPlan.value?.ai_trading_plan_duration.some(
          (existingDuration) => existingDuration.duration_id === duration.value,
        ),
    )
  } else {
...
const create = handleSubmit(async (values: FormInput) => {
  if (!selectedDurations.value.length) {
    toast.dangerText('Please select at least one duration')
    return
  }
  try {
    let data = {
    ...
    if (imageFile.value) {
      ...
      body.append('type', 'aiTradingPlan')
    ...
    const response = await updateAITradingPlan(Number(id), data)
...
<template>
  <TairoContentWrapper>
    <template #left>
      <BaseHeading size="lg">{{ $t('Create AI Trading Plan') }}</BaseHeading>
    </template>
    <template #right>
      <BaseButton
        type="button"
        ...
        to="/admin/extensions/ai-trading/plans"
      >
