...
definePageMeta({
  permissions: ['View AI Trading Investment Durations'],
  title: 'AI Trading Durations',
})
...
const { createAITradingDuration, deleteAITradingDuration } = useAiTrading()
const aiTradingDurationStore = useAiTradingDurationStore()
const durations = computed(() => aiTradingDurationStore.durations)
...
const create = handleSubmit(async (values) => {
  ...
  try {
    const response = await createAITradingDuration(data)
...
async function deletePlan() {
  ...
  try {
    const response = await deleteAITradingDuration(selectedItem.value?.id)
    toast.response(response)
    if (response.status === 'success') {
      aiTradingDurationStore.removeDuration(selectedItem.value?.id)
    }
  } catch (error) {
    toast.danger(error as any)
  }
      ...
      <template #header>
        <div class="flex w-full items-center justify-between p-4 md:p-6">
          <h3
            class="font-heading text-muted-900 text-lg font-medium leading-6 dark:text-white"
          >
            {{ $t('Delete') }} {{ $t('AI Trading Duration') }}
          </h3>
