...
definePageMeta({
  ...
  permissions: ['Edit Staking Pools'],
})
...
const { updateAdminStakingPool } = useStaking()

const existingPool = computed(
  () => stakingPoolStore.selectedPool || stakingPoolStore.getPoolById(id),
)
...
onMounted(async () => {
  ...
  await stakingPoolStore.selectPoolById(methodId)
...
const update = handleSubmit(async (values: FormInput) => {
  try {
    const response = (await updateAdminStakingPool(
      Number(id),
      values.name,
      values.description,
      values.currency,
...
<template>
  <TairoContentWrapper>
    <template #left>
      <BaseHeading size="lg" :key="existingPool?.name">{{
        $t(`
        Editing ${existingPool?.name ?? 'Staking Pool'} Pool
      `)
      }}</BaseHeading>
    </template>
    <template #right>
