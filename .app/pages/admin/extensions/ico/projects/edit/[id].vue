...
const { updateIcoProject } = useIco()

const existingProject = computed(
  () => icoProjectStore.selectedProject || icoProjectStore.getProjectById(id),
)
...
onMounted(async () => {
  ...
  await icoProjectStore.selectProjectById(methodId)
  imagePreviewUrl.value = existingProject.value?.image || null
})
...
const imagePreviewUrl = ref<string | null>(existingProject.value?.image || null)
...
const update = handleSubmit(async (values: FormInput) => {
  try {
    ...
    const response = await updateIcoProject(
      Number(id),
      data.name,
      data.description,
      data.website,
...
<template>
  <TairoContentWrapper>
    <template #left>
      <BaseHeading size="lg" :key="existingProject?.name">{{
        $t(`
        Editing ${existingProject?.name ?? 'ICO Project'} Project
      `)
      }}</BaseHeading>
    </template>
    <template #right>
