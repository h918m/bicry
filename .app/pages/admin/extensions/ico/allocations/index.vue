...
definePageMeta({
  permissions: ['View ICO Allocations'],
  title: 'ICO Allocations',
})

const icoAllocationStore = useIcoAllocationStore()
const { deleteIcoAllocation } = useIco()
...
const items = computed(() =>
  icoAllocationStore.allocations.filter((item) =>
...
onMounted(async () => {
  if (icoAllocationStore.allocations.length === 0) {
    icoAllocationStore.loading = true
    await icoAllocationStore.fetchIcoAllocations()
    icoAllocationStore.loading = false
  }
})
const { toast } = useUtils()
const router = useRouter()

function selectAllocation(item: IcoAllocation) {
  icoAllocationStore.selectAllocation(item)
  router.push(`/admin/extensions/ico/allocations/edit/${item.id}`)
}
...
async function deleteAllocation() {
  ...
  try {
    const response = await deleteIcoAllocation(selectedItem.value?.id)
    toast.response(response)
    if (response.status === 'success') {
      icoAllocationStore.removeAllocation(selectedItem.value?.id)
    }
  } catch (error) {
  ...
  <div>
    <TairoContentWrapper>
      ...
      <template #right>
        <BaseButton
          color="primary"
          :to="`/admin/extensions/ico/allocations/create`"
        >
      ...
      <div>
        <template
          v-if="!icoAllocationStore.loading && paginatedItems?.length === 0"
        >
          <BasePlaceholderPage
            :title="
