...
definePageMeta({
  permissions: ['View ICO Tokens'],
  title: 'ICO Tokens',
})

const icoTokenStore = useIcoTokenStore()
const { deleteIcoToken } = useIco()
...
onMounted(async () => {
  if (icoTokenStore.tokens.length === 0) {
    icoTokenStore.loading = true
    await icoTokenStore.fetchIcoTokens()
    icoTokenStore.loading = false
  }
})
...
function selectToken(item: IcoToken) {
  icoTokenStore.selectToken(item)
  router.push(`/admin/extensions/ico/tokens/edit/${item.id}`)
}
...
async function deleteToken() {
  ...
  try {
    const response = await deleteIcoToken(selectedItem.value?.id)
    toast.response(response)
    if (response.status === 'success') {
      icoTokenStore.removeToken(selectedItem.value?.id)
    }
  } catch (error) {
...
<template>
  <div>
    <TairoContentWrapper>
      ...
      <template #right>
        <BaseButton color="primary" :to="`/admin/extensions/ico/tokens/create`">
          <Icon name="lucide:plus" size="16" class="mr-2" />
          {{ $t('Create') }}</BaseButton
        >
      </template>
      <div>
        <template v-if="!icoTokenStore.loading && paginatedItems?.length === 0">
          <BasePlaceholderPage
            :title="
