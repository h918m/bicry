...
const items = computed(
  () =>
    adminProductsStore.products.filter((item) =>
...
function selectProduct(item: EcommerceProduct) {
  adminProductsStore.selectProduct(item)
  router.push(`/admin/extensions/ecommerce/products/edit/${item.id}`)
}
...
async function deleteSelectedProduct() {
  ...
  try {
    const response = await deleteAdminProduct(selectedItem.value?.id)
    toast.response(response)
    if (response.status === 'success') {
      adminProductsStore.removeProduct(selectedItem.value?.id)
    }
  } catch (error) {
  ...
  <div>
    <TairoContentWrapper>
      ...
      <template #right>
        <BaseButton
          color="primary"
          :to="`/admin/extensions/ecommerce/products/create`"
        >
          <Icon name="lucide:plus" size="16" class="mr-2" />
          {{ $t('Create Product') }}</BaseButton
        >
      </template>
      <div>
        <template
          v-if="!adminProductsStore?.loading && paginatedItems?.length === 0"
        >
          <BasePlaceholderPage
            :title="
