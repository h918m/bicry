...
definePageMeta({
  title: 'Update Forex Plan',
  permissions: ['Update Forex Plans'],
})
...
const { updateForexPlan } = useForex()

const existingPlan = computed(
  () => forexPlanStore.selectedPlan || forexPlanStore.getPlanById(id),
)
...
onMounted(async () => {
  ...
  await forexPlanStore.selectPlanById(methodId)
  imagePreviewUrl.value = existingPlan.value?.image || null
  if (!forexPlanStore.selectedPlan) {
  ...
  if (existingPlan.value?.forex_plan_duration) {
    selectedDurations.value = durations.value.filter(
      (duration) =>
        existingPlan.value?.forex_plan_duration.some(
...
const create = handleSubmit(async (values: FormInput) => {
  ...
  try {
    ...
    const response = await updateForexPlan(Number(id), data)
